...
class DualResNet(nn.Module):
    def __init__(self, input_channels=64):
        ...
        self.resnet152 = resnet_152
        self.resnet50 = wpt_resnet_50(input_channels=input_channels)
    ...
    def forward(self, x, y):
        ...
        y = self.resnet50.layer1(y)
        y = self.resnet50.layer2(y)
        y = self.resnet50.layer3(y)
        y = self.resnet50.layer4(y)
        y = self.resnet50.avgpool(y)
        y = torch.flatten(y, 1)
        y = self.resnet50.fc(y)
...
# class WPTDenseNet(models.DenseNet):
#     def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
#                  num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, memory_efficient=False,
#                  input_channels=64):
#
...
#
#         if input_channels < 64:
#             out_ch = self.features.denseblock1.denselayer1.conv1.out_channels
#             kernel_size = self.features.denseblock1.denselayer1.conv1.kernel_size
#             temp_layer = nn.Conv2d(input_channels, out_ch, kernel_size=kernel_size, stride=1,
#                                    bias=False)
#             # NOTE: In DCT source code, res50 has a bug here, kernel size = 3, but when copying weight,
#             # kernel_size became 1. They probably mixed res18 and res50 by mistake
#             temp_layer.weight.data = self.features.denseblock1.denselayer1.conv1.weight.data[:, :input_channels]
#             self.features.denseblock1.denselayer1.conv1 = temp_layer
#
#             # although conventional res18's BasicBlock doesn't have 'downsample',
#             # we do need downsample here, otherwise,
#             # the residual add operation will throw error
...
# def _wptdensenet121(arch, growth_rate, block_config, num_init_features, pretrained, progress,
#               **kwargs):
#     model = WPTDenseNet(growth_rate, block_config, num_init_features, **kwargs)
#     return model

# def wptdensenet121(input_channels, **kwargs):
#     model = WPTDenseNet(input_channels=input_channels, **kwargs)
#     return model
